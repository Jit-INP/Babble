...
sem_t fullCount, emptyCount, com_mutex;
int in, out;
...
command_t* get_command()
{
    sem_wait(&fullCount);
    sem_wait(&com_mutex);
    command_t* cmd = command_buffer[out];
...
int main(int argc, char *argv[])
{
    ...
    pthread_t exec_tids[NB_EXE_THREADS];
    int tid = 0;
    ...
    if(sem_init(&com_mutex, 0, 1) == 1)
      printf("Error on com_mutex semaphore\n");
    ...
    for(int i = 0; i < NB_EXE_THREADS; i++)
    {
        if(pthread_create (&exec_tids[i], NULL, execution_thread, NULL) != 0){
